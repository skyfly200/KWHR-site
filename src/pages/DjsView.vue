  ...
  <v-container style="max-width: 1200px" class="pb-12">
    <v-row>
      <v-col v-for="dj in active" :key="dj.slug" cols="12" sm="6" md="4">
        <v-card :to="`/djs/${dj.slug}`" class="pa-6 h-100 lift" rounded="xl">
          <v-avatar :color="avatarColor(dj.name)" size="64" class="mb-4">
    ...
    <template v-if="alumni.length">
      ...
      <v-row>
        <v-col v-for="dj in alumni" :key="dj.slug" cols="6" sm="4" md="3">
          <v-card :to="`/djs/${dj.slug}`" class="pa-4 h-100 lift text-center" rounded="xl">
            <v-avatar :color="avatarColor(dj.name)" size="44" class="mb-2">
              <span class="text-body-2 font-weight-bold" style="color: #fff">{{ initials(dj.name) }}</span>
            </v-avatar>
            <div class="text-body-2 font-weight-bold">{{ dj.name }}</div>
          </v-card>
        </v-col>
      </v-row>
    </template>
